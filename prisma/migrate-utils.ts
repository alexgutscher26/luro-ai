...
export const MIGRATION_CONFIGS: Record<'development' | 'staging' | 'production', MigrationConfig> = {
  development: {
...
export class MigrationManager {
  ...
  private getConfig(environment: string): MigrationConfig {
    return MIGRATION_CONFIGS[environment as keyof typeof MIGRATION_CONFIGS] ?? MIGRATION_CONFIGS.development;
  }
  ...
  async checkMigrationStatus(): Promise<{
    ...
    try {
      ...
      const pendingMigrations = this.extractPendingMigrations(statusOutput);
      const appliedMigrations = this.extractAppliedMigrations(statusOutput);
  ...
  async deployMigrations(): Promise<void> {
    ...
    if (this.config.requireApproval && !process.env.MIGRATION_APPROVED) {
      throw new Error('Migration requires approval. Set MIGRATION_APPROVED=true to proceed.');
    }
  ...
  private extractPendingMigrations(statusOutput: string): string[] {
    ...
    const pendingSection = lines.findIndex(line => line.includes('pending'));
  ...
  private extractAppliedMigrations(statusOutput: string): string[] {
    const lines = statusOutput.split('\n');
    const appliedSection = lines.findIndex(line => line.includes('applied'));
  ...
  private async addMigrationDescription(migrationName: string, description: string): Promise<void> {
