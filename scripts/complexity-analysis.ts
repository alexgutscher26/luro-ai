...
class CodeComplexityAnalyzer {
    ...
    private calculateHalsteadMetrics(content: string): HalsteadMetrics {
        ...
        const difficulty =
            (uniqueOperators / 2) * (operands.length / uniqueOperands);
        ...
        try {
            ...
            if (fs.existsSync(coverageFile)) {
                ...
                if (fileCoverage) {
                    ...
                    const covered = Object.values(s).filter(
                        (count: any) => count > 0
                    ).length;
            ...
            .filter(
                m =>
                    m.cyclomaticComplexity >
                        this.complexityThresholds.cyclomatic ||
                    m.cognitiveComplexity > this.complexityThresholds.cognitive
            )
            .map(m => m.file);

        const averageMaintainability =
            metrics.reduce((sum, m) => sum + m.maintainabilityIndex, 0) /
            totalFiles;
        ...
        return {
            summary: {
                ...
                maintainabilityScore:
                    Math.round(averageMaintainability * 100) / 100,
    ...
    private generateRecommendations(metrics: ComplexityMetrics[]): string[] {
        ...
        const highComplexityFiles = metrics.filter(
            m => m.cyclomaticComplexity > this.complexityThresholds.cyclomatic
        );
        if (highComplexityFiles.length > 0) {
            recommendations.push(
                `🔴 ${highComplexityFiles.length} files have high cyclomatic complexity (>${this.complexityThresholds.cyclomatic}). Consider refactoring.`
            );
        }

        const lowMaintainabilityFiles = metrics.filter(
            m =>
        ...
        if (lowMaintainabilityFiles.length > 0) {
            recommendations.push(
                `🟡 ${lowMaintainabilityFiles.length} files have low maintainability index (<${this.complexityThresholds.maintainability}). Review and simplify.`
            );
        }
        ...
        if (highDependencyFiles.length > 0) {
            recommendations.push(
                `🟠 ${highDependencyFiles.length} files have high dependency count (>15). Consider dependency injection or modularization.`
            );
        }

        const uncoveredFiles = metrics.filter(
            m => m.testCoverage !== undefined && m.testCoverage < 80
        );
        if (uncoveredFiles.length > 0) {
            recommendations.push(
                `🔵 ${uncoveredFiles.length} files have low test coverage (<80%). Add more tests.`
            );
        }

        if (recommendations.length === 0) {
            recommendations.push(
                "✅ Code complexity is within acceptable ranges. Great job!"
            );
        }
    ...
    private calculateTechnicalDebt(metrics: ComplexityMetrics[]): string {
        const totalComplexity = metrics.reduce(
    ...
    private async saveReport(report: ComplexityReport): Promise<void> {
        ...
        const reportPath = path.join(
            reportsDir,
            `complexity-report-${timestamp}.json`
        );
...
if (require.main === module) {
    ...
    analyzer
        ...
        .then(report => {
            ...
            console.log(
                `🔧 Maintainability score: ${report.summary.maintainabilityScore}`
            );
