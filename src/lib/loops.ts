...
const getLoopsClient = () => {
  ...
  if (!apiKey) {
    console.warn('LOOPS_API_KEY is not set. Loops integration will be disabled.');
    return null;
  }
...
export async function createLoopsContact({
  ...
  try {
    ...
    if (!loopsClient) {
      console.warn('Loops client not available - skipping contact creation');
      return null;
    }
    ...
    const response = await loopsClient.createContact(email, contactProperties);
